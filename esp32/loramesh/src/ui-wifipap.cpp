...
void clicked(Button2& btn)
{
  ...
  if (lora_selection) { // cycle through the choices
    ...
    ((UI_WifiPaper_Class*)theUI)->to_next_screen();
  }
}
...
void long_clicked(Button2& btn)
{
  ...
  if (lora_selection) { // this is our choice
    ...
    UI_WifiPaper_Class *u = (UI_WifiPaper_Class*) theUI;
    if ( u->curr_screen == UI_WifiPaper_Class::SCREEN_LORA) {
...
UI_WifiPaper_Class::UI_WifiPaper_Class()
{

#define INFLATE(NM) inflate(NM##_bw, sizeof(NM##_bw),   \
                            NM##_z, sizeof(NM##_z));
  // INFLATE(scuttleshell)
  curr_screen = UI_WifiPaper_Class::SCREEN_SPLASH;
...
void UI_WifiPaper_Class::loop()
{
  ...
  if (curr_screen == UI_WifiPaper_Class::SCREEN_SPLASH) {
  ...
  if (curr_screen == UI_WifiPaper_Class::SCREEN_PEERS) {
    static long next;
    if (millis() > next) {
      refresh_screen(curr_screen);
      next = millis() + 900;
...
void UI_WifiPaper_Class::refresh()
{
  refresh_screen(curr_screen);
}


void UI_WifiPaper_Class::to_next_screen()
{
  curr_screen = (curr_screen + 1) % UI_WifiPaper_Class::SCREEN_OFF;
  refresh_screen(curr_screen);
}
...
void UI_WifiPaper_Class::refresh_screen(int scr)
{
  if (scr != curr_screen)
      return;
  ...
  theDisplay.clear();
  theDisplay.setFont(ArialMT_Plain_16);
  if (scr == UI_WifiPaper_Class::SCREEN_SPLASH) {
...
#endif
  } else if (scr == UI_WifiPaper_Class::SCREEN_NODE) {
    theDisplay.setFont(ArialMT_Plain_10);
  ...
  } else if (scr == UI_WifiPaper_Class::SCREEN_LORA) {
#ifdef HAS_LORA
    draw_lora_plan();
#else
    theDisplay.drawString(2, 20, "lora screen");
#endif
  } else if (scr == UI_WifiPaper_Class::SCREEN_PEERS) {
    theDisplay.setFont(ArialMT_Plain_10);
...
void UI_WifiPaper_Class::show_boot_msg(char *s)
{
#ifdef HAS_OLED
