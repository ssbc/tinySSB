...
function backend(cmdStr) { // send this to Kotlin (or simulate in case of browser-only testing)
    ...
    if (cmdStr == 'ready' && window.location.search == '?virtualBackend=true')
        return;
    if (virtBackEnd != null) {
...
function b2f_new_event(e) { // incoming SSB log event: we get map with three entries
    ...
    console.log("New frontend event, hdr=" + JSON.stringify(e.header))
...
// >> BEGIN INSERT VIRTBACKEND
var virtBackEnd = null;
var cnt = 0;

window.addEventListener('message', function(event) { 
    if (virtBackEnd == null)
        virtBackEnd = event.source;
    if (event.data[0] != 'b2f') // event.data[0] == 'start' || 
        return false;
    console.log('tremola: event data ', event.data);

    let cmd = event.data[1];
    let dat = event.data[2];
